...
TEST(Example01, main)
{
    ...
    order_id.Seller02 = table.fx_fgApi_order_add(user_id.Seller02, EnumOrderSide::SELLER, EnumOrderTimeInForce::NORMAL, 999, 50).value_or(0);  // Seller02
    ...
    if (not stub_tmp.lock()->empty())
    {
        ...
        table.fx_fgApi_suborder_match_request_response(         //
            user_id.Seller01, suborder_id[user_id.Seller01], 90 //
        );
    }
    ...
    if (not stub_tmp.lock()->empty())
    {
        suborder_id = func(stub_tmp.lock().operator*());
        table.fx_fgApi_suborder_match_request_response(         //
            user_id.Seller01, suborder_id[user_id.Seller01], 10 //
        );
    }
